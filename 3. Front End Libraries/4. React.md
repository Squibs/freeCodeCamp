...
const renderFrameworks = frontEndFrameworks.map((value, index) => <li key={generateKey(`${index}_${value}`)}>{value}</li>);
```
...
class MyComponent extends React.Component {
  ...
  render() {
    ...
    const renderOnline = usersOnline.map((user, index) => <li key={`${index}_${user.username}_${new Date().getTime()}`}>{user.username}</li>); // change code here
    return (
      <div>
