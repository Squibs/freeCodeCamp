...
```
The `<em>` (emphemeral unit) tag can be used to display text as italicized. It can be used to give text emphasis.
...
<div id="rect"></div>
```
Keyframes are used a lot in animation. Keyframes are like specified positions you want elemetns to be in a specific times. The frames inbetween are generated automatically via interpolation.
