...
This is not used to increase browser compatibility. This instead is used as a `fallback` in-case the browser cannot find your variable.
...
Not all browsers support the same CSS properties. For example CSS `custom properties` or `CSS variables` might not be supported on every browser. Assigning a property, and then reassigning the same property, can allow for fallbacks for browsers that might not support a specific property.

In this challenge, this is demonstrated by first giving the background property the color red, as a `default/fallback`. Then reassigning the background property using a `CSS variable`, for use in browsers that support `CSS variables`.
